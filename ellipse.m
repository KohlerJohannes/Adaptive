function h=ellipse(ra,rb,ang,x0,y0,C,Nb,Fill)
...
for k=1:maxk
  ...
  xx=radm*cos(the)*co-si*radn*sin(the)+xpos;
  yy=radm*cos(the)*si+co*radn*sin(the)+ypos;
  
  if Fill==0
      p=line(xx,yy);
      set(p,'color',C(rem(k-1,size(C,1))+1,:));
  else
      p = fill(xx,yy,C(rem(k-1,size(C,1))+1,:));
      p.FaceColor = C(rem(k-1,size(C,1))+1,:);  
      p.EdgeColor = 'none';
  end
